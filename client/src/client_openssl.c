...
int init_ssl(client_t* client)
{
    ...
    if (!client->ssl_ctx)
    {
        log_message(LOG_ERROR, CLIENT_LOG, __FILE__, "Failed to create SSL context");
        finish_logging();
    ...
    if (!client->ssl)
    {
        log_message(LOG_ERROR, CLIENT_LOG, __FILE__, "Failed to create SSL object");
        finish_logging();
        destroy_ssl(client);
        return OPENSSL_SSL_OBJECT_FAILURE;
    }
    SSL_set_fd(client->ssl, client->socket);
    log_message(LOG_INFO, CLIENT_LOG, __FILE__, "SSL initialization successful");

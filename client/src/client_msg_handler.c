...
{
    // handle PING and ACK messages
    if (msg->type == MESSAGE_PING)
    {
        create_message(msg, MESSAGE_ACK, client->uid, "server", "ACK"); // [ ] handle ACK from other users
    ...
    if (msg->type == MESSAGE_AUTH && !strcmp(msg->payload, message_code_to_string(MESSAGE_CODE_ENTER_USERNAME)))
    {
        state->just_joined = 0;
